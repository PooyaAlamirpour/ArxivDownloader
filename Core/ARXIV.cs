...
namespace ARXIVDownloader.Core
{
    class ARXIV : Download, IARXIV
    {
        ...
        internal void DownloadAllLinks()
        {
            ...
            if(!path.Equals("NaN"))
            {
                if(Directory.Exists(path))
                {
                    if (remainedArticles.Count == 0)
                    {
                        ...
                        if (results > 200)
                        {
                            CollectAndDownloadLinksInSinglePage(this.content);
            ...
            if(remainedArticles.Count > 0)
            {
                PDFList = remainedArticles;
                this.form1.UpdateDownloadButton(remainedArticles.Count.ToString() + " remained", true);
            }
            else
            {
                PDFList = new List<string>();
                remainedArticles = new List<string>();
                this.form1.UpdateDownloadButton("Downloaded", false);
            }
        }
        ...
        private void CollectAndDownloadLinksInSinglePage(String input)
        {
        ...
        public List<string> remainedArticles = new List<string>();
        ...
        private async void DownloadSignleFile(String address)
        {
            ...
            using (WebClient client = new WebClient())
            {
                try
                {
                    string ArticleName = filename.Replace(":", ",").Replace("?", "").Replace("/", "-") + ".pdf";
                    if (!File.Exists(path + "\\" + ArticleName))
                    {
                        client.Headers.Add("User-Agent: Other");
                        client.DownloadProgressChanged += wc_DownloadProgressChanged;
                        client.DownloadFileCompleted += wc_DownloadFileCompleted;
                        await client.DownloadFileTaskAsync(new Uri(address), path + "\\" + ArticleName);
                    }
                    else
                    {
                        log.Show(ArticleName + " exists.");
                    }
                }
                catch (Exception ex)
                {
